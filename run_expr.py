...
Run command:
	CUDA_VISIBLE_DEVICES=0 python run_expr.py --kernel=Gaussian --s=5 --mem_gb=12 --epochs 1 2 3 4 5
'''
...
parser.add_argument('-k', '--k', type=np.int32, default=None,
                    help="using the top-k eigensystem for the adaptive kernel")
...
model = AdaptiveKernel(kernel, x_train, n_class,
                       mem_gb=args_dict['mem_gb'],
                       n_subsample=args_dict['n_subsample'],
                       k=args_dict['k'],
                       bs=args_dict['bs'])
model.fit(x_train, y_train, x_test, y_test, args_dict['epochs'])
